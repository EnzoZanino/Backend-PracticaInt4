...
export default class CourseService {
	...
	save = async (curso) => {
		...
		try {
			...
			console.error(
				`Error guardando recurso: ${JSON.stringify(
					usuarioNuevo
	...
	getAll = async () => {
		try {
			...
			console.error(`Error consultando los cursos por archivo, valide el archivo: ${
				this.#dirPath
			}, 
                detalle del error: ${error}`);
			throw Error(`Error consultando los cursos por archivo, valide el archivo: ${
				this.#dirPath
			},
             detalle del error: ${error}`);
		}
